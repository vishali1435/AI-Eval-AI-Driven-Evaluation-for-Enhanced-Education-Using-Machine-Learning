...
def connect_to_db():
    db = pymysql.connect("localhost", "root", "mite", "aep")
    cursor = db.cursor()  # Fixed the indentation to match the rest of the block
...
sql = "UPDATE answer SET m1 = %s, m2 = %s, m3 = %s, m4 = %s, m5 = %s, m6 = %s, total = %s WHERE usn = %s"
val = (
    total_marks_in_a_subject[0],  # m1
    total_marks_in_a_subject[1],  # m2
    total_marks_in_a_subject[2],  # m3
    total_marks_in_a_subject[3],  # m4
    total_marks_in_a_subject[4],  # m5
    total_marks_in_a_subject[5],  # m6
    total_marks_of_student,       # total
    student_usn                   # usn
)
cursor.execute(sql, val)
db.commit()
